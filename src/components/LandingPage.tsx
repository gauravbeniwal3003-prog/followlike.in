...
export default function LandingPage({
  ...
  return (
    <div id="landing-root" className="min-h-screen relative overflow-hidden bg-neutral-950 text-white font-sans selection:bg-white selection:text-black">
      ...
      {/* Services Table and Rates catalog */}
      <section id="catalog" className="py-20 border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-10 gap-4">
            <div>
              <span className="text-xs font-mono text-neutral-500 tracking-widest uppercase">Real-Time catalog</span>
              <h2 className="text-3xl font-semibold tracking-tight text-white mt-1">Our Interactive Rates Matrix</h2>
              <p className="text-xs text-neutral-400 mt-1">Browse pricing per 1,000 units before completing your secure login.</p>
            </div>
            ...
            <div className="w-full md:w-80">
              <input
                ...
                type="text"
                placeholder="Search Instagram, Subscribers, Views..."
                value={searchQuery}
          ...
          <div className="flex flex-wrap gap-1.5 mb-6 border-b border-white/[0.05] pb-4">
            {categories.map((cat) => (
              <button
                key={cat}
                id={`catalog-category-tab-${cat.replace(/\s+/g, '-').toLowerCase()}`}
          ...
          {/* Table Container in Liquid Glass card */}
          <div className="overflow-hidden border border-white/5 bg-white/[0.01] rounded-xl">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="border-b border-white/[0.06] bg-white/[0.02] text-neutral-400 font-mono text-[10px] uppercase">
                    <th className="p-4">Platform</th>
                    <th className="p-4">Service Details</th>
                    <th className="p-4 text-right">Rate/1k USD</th>
                    <th className="p-4 text-center">Min Order</th>
                    <th className="p-4 text-center">Max Order</th>
                  ...
                  {filteredServices.length > 0 ? (
                    filteredServices.map((service) => (
                      <tr key={service.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="p-4 font-semibold text-white whitespace-nowrap">
                          <span className="px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/10 p-1 font-mono text-[10px]">
                            {service.category}
                          </span>
                        </td>
                        <td className="p-4 max-w-sm">
                          <div className="font-semibold text-white group-hover:text-white transition-colors">{service.name}</div>
                          {service.description && (
                            <p className="mt-1 text-[11px] text-neutral-400 leading-relaxed font-sans">{service.description}</p>
                          )}
                        </td>
                        <td className="p-4 text-right font-mono font-medium text-white">${service.ratePer1000.toFixed(2)}</td>
                        <td className="p-4 text-center font-mono text-neutral-400">{service.min.toLocaleString()}</td>
                        <td className="p-4 text-center font-mono text-neutral-400">{service.max.toLocaleString()}</td>
                        <td className="p-4 text-right">
                          <button
                            id={`catalog-buy-btn-${service.id}`}
                            onClick={() => setIsGmailModalOpen(true)}
                            className="px-3 py-1 rounded bg-white text-black font-semibold hover:bg-neutral-200 transition-colors cursor-pointer text-[11px]"
                          >
                            Buy
                          </button>
                        </td>
                    ...
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-neutral-500 font-mono text-xs">
                        No services matching application parameters. Try resetting your search query.
                      </td>
                    </tr>
                  )}
                </tbody>
